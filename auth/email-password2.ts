...
const anonLinkButton = document.getElementById(
    'quickstart-anon-link',
...
/**
 * Handles the anon link button press.
 */
function handleAnonLink() {
  const email = emailInput.value;
  const password = passwordInput.value;
  if (email.length < 4) {
    alert('Please enter an email address.');
...
anonLinkButton.addEventListener('click', handleAnonLink, false);
